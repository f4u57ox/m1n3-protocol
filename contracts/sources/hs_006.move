/// HashShare slot 006 — one of the pre-published `Coin<HASHSHARE_*>` types
...
module m1n3_v4::hs_006 {
    use sui::coin;

    public struct HS_006 has drop {}

    fun init(witness: HS_006, ctx: &mut TxContext) {
        let (cap, meta) = coin::create_currency<HS_006>(
            witness,
            0,                 // decimals: HashShares are integer counts of difficulty
            b"HS006",
            b"m1n3 HashShare 000",
            b"Per-round tokenized mining share, slot 006",
            option::none(),
            ctx,
        );
    ...
    #[test_only]
    public fun init_for_testing(ctx: &mut TxContext) { init(HS_006 {}, ctx); }
}
