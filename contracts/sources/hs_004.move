/// HashShare slot 004 — one of the pre-published `Coin<HASHSHARE_*>` types
...
module m1n3_v4::hs_004 {
    use sui::coin;

    public struct HS_004 has drop {}

    fun init(witness: HS_004, ctx: &mut TxContext) {
        let (cap, meta) = coin::create_currency<HS_004>(
            witness,
            0,                 // decimals: HashShares are integer counts of difficulty
            b"HS004",
            b"m1n3 HashShare 000",
            b"Per-round tokenized mining share, slot 004",
            option::none(),
            ctx,
        );
    ...
    #[test_only]
    public fun init_for_testing(ctx: &mut TxContext) { init(HS_004 {}, ctx); }
}
