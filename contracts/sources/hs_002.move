/// HashShare slot 002 — one of the pre-published `Coin<HASHSHARE_*>` types
...
module m1n3_v4::hs_002 {
    use sui::coin;

    public struct HS_002 has drop {}

    fun init(witness: HS_002, ctx: &mut TxContext) {
        let (cap, meta) = coin::create_currency<HS_002>(
            witness,
            0,                 // decimals: HashShares are integer counts of difficulty
            b"HS002",
            b"m1n3 HashShare 000",
            b"Per-round tokenized mining share, slot 002",
            option::none(),
            ctx,
        );
    ...
    #[test_only]
    public fun init_for_testing(ctx: &mut TxContext) { init(HS_002 {}, ctx); }
}
